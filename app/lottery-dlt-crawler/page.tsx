...
export default function DLTCrawlerPage() {
  ...
  // 号码搜寻状态（长度为 7，前 5 个是红球，第 6、7 个是蓝球）
  ...
  if (userLoading) {
    return (
      <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-amber-100/20 blur-[120px] rounded-full pointer-events-none animate-pulse" />
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 selection:bg-amber-500/10 relative overflow-x-hidden">
      {/* 氛围背景微网格与柔和渐变 */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-amber-100/20 blur-[130px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-emerald-100/20 blur-[130px] rounded-full" />
        <div
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-12 border-b border-slate-200/60 pb-8">
          <div className="space-y-3">
            ...
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900 leading-none">
              大乐透 <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-600 via-orange-500 to-emerald-600">智能对奖端</span>
            </h1>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                ...
                <label className="flex items-center gap-2.5 cursor-pointer group">
                  <div className={`w-9 h-5.5 flex items-center rounded-full transition-colors duration-300 ${isExactMatch ? 'bg-amber-500 shadow-sm shadow-amber-100' : 'bg-slate-200'}`}>
              ...
              <div className="space-y-4 py-4 bg-slate-50/50 rounded-2xl p-5 border border-slate-100/80">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 w-full">
                  ...
                  <LotteryNumbersInput
                    lotteryType="dlt"
                    redNumbers={searchDigits.slice(0, 5)}
                    blueNumbers={searchDigits.slice(5)}
                    onChange={(red, blue) => {
                ...
                <p className="text-xs text-slate-400 font-normal pl-0 sm:pl-16 tracking-tight leading-relaxed">
                  💡 提示：支持从网页或文本直接复制整行由空格或逗号分隔的号码，直接粘贴至第一个前区格，系统会自动进行智能拆分填充。
                </p>
              </div>
        ...
        <div className="mb-12 flex justify-start">
          {(() => {
            ...
            if (isPending) {
              ...
              statusBg = "bg-rose-50/80 border-rose-100/80 text-rose-850 shadow-[0_2px_12px_rgba(225,29,72,0.03)]";
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-bold text-amber-500">守护中</span>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {ticket.numbers?.blue?.map((num: string, idx: number) => (
                        <div key={idx} className="w-8 h-8 rounded-lg bg-emerald-50 border border-emerald-100/80 flex items-center justify-center text-xs font-bold text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white group-hover:border-emerald-600 transition-all duration-300">
                          {num}
                        </div>
                      ))}
                    </div>
                            ...
                            <div className="flex flex-wrap items-center gap-1.5">
                              {item.openNumbers?.red?.map((num: string, idx: number) => {
                                ...
                                return (
                                  ...
                                  >
                                    {num}
                                    {isActive && <div className="absolute -inset-0.5 bg-amber-400/20 blur-sm rounded-lg animate-pulse" />}
                                  </div>
                                );
                              })}
                              ...
                              {item.openNumbers?.blue?.map((num: string, idx: number) => {
                                ...
                                return (
                                  ...
                                  >
                                    {num}
                                    {isActive && <div className="absolute -inset-0.5 bg-emerald-400/20 blur-sm rounded-lg animate-pulse" />}
                                  </div>
                                );
                              })}
                            </div>
                            ...
                            <button
                              onClick={() => {
                                setSelectedPrizeInfo({
                                  ...
                                  prizeAmounts: item.prizeAmounts || null,
                                });
                                setIsDialogOpen(true);
                              }}
                              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-255/60 text-slate-500 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-200 transition-all font-bold text-xs shadow-sm"
                            >
                              <span>查看详情</span>
              ...
              {selectedPrizeInfo?.prizeAmounts && selectedPrizeInfo.prizeAmounts.length > 0 ? (
                selectedPrizeInfo.prizeAmounts.map((prize, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50 border border-slate-200/60 shadow-sm transition-all hover:bg-white hover:border-slate-300">
                    ...
                    <span className="text-xs font-mono font-bold text-amber-600 bg-amber-50 border border-amber-100/50 px-2.5 py-1 rounded-md">{prize.amount}</span>
                  </div>
                ))
              ) : (
