...
export default function DLTCrawlerPage() {
  ...
  const searchBlueNumbersSet = useMemo(() => {
    if (!isSearching || !searchInput.trim()) return new Set<string>();
    const numbers = parseSearchNumbers(searchInput)
      .slice(5, 7)
      .map((n) => {
  ...
  const isNumberMatched = (num: string | number, isRed: boolean): boolean => {
    ...
    if (isRed) {
      ...
      return searchBlueNumbersSet.has(normalizedNum);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-200 selection:bg-amber-500/30">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-amber-900/10 blur-[120px] rounded-full animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-emerald-900/10 blur-[120px] rounded-full animate-pulse" style={{ animationDelay: '2s' }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto p-4 md:p-12 animate-in fade-in slide-in-from-bottom-12 duration-1000">
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-16">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[10px] uppercase tracking-[0.2em] font-bold">
            ...
            <h1 className="text-5xl md:text-6xl font-black tracking-tighter text-white">
              大乐透 <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 via-orange-500 to-yellow-500">获取引擎</span>
            </h1>
            <p className="text-slate-500 text-lg font-light max-w-md">
              超级大乐透开奖数据深度追踪与模式干预分析系统
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              ...
              <span className="relative flex items-center gap-2">
                切换双色球节点 <span className="group-hover:translate-x-1 transition-transform">→</span>
              </span>
            </button>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-12">
          ...
          <section className="lg:col-span-3 p-8 rounded-[2.5rem] bg-white/[0.03] border border-white/[0.06] backdrop-blur-2xl shadow-2xl space-y-8">
            <div className="flex flex-wrap items-center gap-6">
              <div className="space-y-1.5 min-w-[120px]">
                ...
                <select
                  ...
                  className="w-full h-12 px-4 rounded-xl bg-black/40 border border-white/10 text-white outline-none cursor-pointer focus:border-amber-500/50 transition-all appearance-none"
                >
                  {Array.from(
              ...
              <div className="flex items-end gap-3 flex-1">
                ...
                <button
                  ...
                  disabled={loading}
                  className="px-6 h-12 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold disabled:opacity-50 transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] active:scale-[0.98]"
                >
                  {loading ? "加载中" : "一键全量同步"}
                </button>
              </div>
            ...
            <div className="relative group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                <svg className="w-5 h-5 text-slate-500 group-focus-within:text-amber-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
              </div>
              <input
                type="text"
                value={searchInput}
                ...
                className="w-full h-14 pl-12 pr-32 rounded-2xl bg-black/40 border border-white/10 text-white placeholder-slate-600 outline-none focus:border-amber-500/50 focus:bg-black/60 transition-all text-lg"
              />
              <div className="absolute inset-y-2 right-2 flex items-center gap-2">
                 <button
                  onClick={handleClearSearch}
                  className={`h-full px-4 rounded-xl text-xs font-bold transition-all ${isSearching ? 'bg-amber-500/20 text-amber-500 opacity-100' : 'opacity-0 pointer-events-none'}`}
                >
                  重置
                </button>
                <button
            ...
            <div className="flex items-center gap-4 px-1">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className={`w-10 h-6 flex items-center rounded-full transition-colors ${isExactMatch ? 'bg-amber-500' : 'bg-slate-800'}`}>
          ...
          <aside className="space-y-6">
            <div className="p-6 rounded-[2rem] bg-gradient-to-br from-amber-500/20 to-emerald-500/20 border border-amber-500/20 backdrop-blur-xl">
              <h3 className="text-xs font-black text-amber-400 uppercase tracking-widest mb-4">系统反馈</h3>
              {result ? (
                <p className="text-sm font-medium leading-relaxed italic text-amber-200">"{result}"</p>
              ) : (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {ticketData.data.list.map((ticket: any) => (
                  <div
                    key={ticket.id}
                    className="group p-5 rounded-3xl bg-white/[0.02] border border-white/[0.05] hover:border-amber-500/30 hover:bg-white/[0.04] transition-all duration-300"
                  >
                        ...
                        <TableRow key={item.issueNumber} className="border-white/[0.04] hover:bg-white/[0.02] transition-colors group">
                          ...
                          <TableCell className="py-6 px-4">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <div className="w-px h-6 bg-white/10 mx-1" />
                              {item.openNumbers?.blue?.map((num: string, idx: number) => {
                                  const matched = isNumberMatched(num, false);
                                  const isActive = isSearching && matched;
                                  return (
                                    <div
                                      key={num + idx}
                                      className={`relative w-9 h-9 flex items-center justify-center rounded-xl text-sm font-black transition-all duration-500 ${
                                        isActive 
                                          ? "bg-emerald-600 text-white shadow-[0_0_20px_rgba(16,185,129,0.5)] scale-110 z-10" 
                                          : isSearching 
                                            ? "bg-slate-900/50 text-slate-700 opacity-40" 
                                    ...
                                    >
                                      {num}
                                       {isActive && <div className="absolute -inset-0.5 bg-emerald-400/20 blur-sm rounded-xl animate-pulse" />}
                                    </div>
                                  );
                                })}
                            </div>
                          ...
                          <TableCell className="py-6 px-8 text-right">
                            <button
                              ...
                              className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white hover:bg-amber-600 hover:border-amber-600 hover:shadow-[0_0_15px_rgba(245,158,11,0.3)] transition-all"
                            >
                              详情
                            </button>
                          </TableCell>
              ...
              {selectedPrizeInfo?.prizeAmounts && selectedPrizeInfo.prizeAmounts.length > 0 ? (
                <div className="space-y-3">
                  {selectedPrizeInfo.prizeAmounts.map((prize: any, idx: number) => (
                    <div key={idx} className="flex justify-between items-center p-5 bg-white/[0.03] rounded-[1.5rem] border border-white/5 hover:border-amber-500/20 transition-colors group">
                      <div className="space-y-1">
                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">奖项能级</div>
                        <div className="text-sm font-bold text-white group-hover:text-amber-400 transition-colors tracking-tight">{prize.level}</div>
                      </div>
