...
JSON 格式要求如下：
{
  ...
  batchPredictMatches: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const budgetAmount = parseInt(input.budget?.match(/\d+/)?.[0] || "100", 10);
...
用户的购彩偏好设定如下：
- 打票总预算上限：${budgetAmount}元（推荐方案的总花费必须【严格小于或等于】${budgetAmount}元，绝不能超支，且口径必须严密吻合）。
- 风险承受等级：${input.risk || "未指定（默认稳妥为主）"}
${input.rules ? `- 额外的自定义出号规则（必须严格遵守此限制条件）：${input.rules}` : ""}

- 竞彩金额与倍数计算规则（非常重要，请计算清楚再输出）：
  - 竞彩每注基础本金为 2 元。
  - 单张票总花费 = 2元 × 该票的总注数 × 倍数。
  - 你的推荐方案（包括所有注数、倍数）的总花费之和必须完全精确等于或小于 ${budgetAmount}元。
  - 例如，如果推荐的是 1 注（单组 4串1 只有 1 注），预算为 10 元，则最大打 5 倍（2元 × 1注 × 5倍 = 10元）。此时 \`howToBuy\` 应该写为：“打5倍（10元）”。绝对不能写“投10元打10倍”（这实际花费20元或100元，超支了），也不能写“投10元打5倍”（口径前后矛盾）。
  - 如果推荐的是 3串4（共 4 注），打 1 倍需要 8 元。若预算为 10 元，则最大只能打 1 倍（花费 8 元，剩余 2 元无法再打一倍，不能超支）。

- 预计中奖金额计算规则（必须严格进行乘法计算，严禁幻觉脑补）：
  - 预计中奖金额必须基于你推荐的具体选项对应的【真实赔率】进行连乘计算。
  - 单组过关中奖奖金 = 所有关卡赔率的乘积 × 2元 × 该方案的倍数。
  - 你在输出 \`whyToBuy\` 中的预计中奖金额时，**必须简要写出你的乘法计算过程，证明你的计算是完全科学和符合数学事实的**。
  - 绝对不可以在没有任何计算依据的情况下，凭空生成如“预计中奖800-1200元”这样严重偏离真实的数字！如果本金是 10 元，4场低赔（比如在1.5-2.5之间）的4串1，最高奖金绝不可能达到几百上千元。请用你的数学乘法做严密的计算。
...
6. **必须严格遵循用户的【风险承受等级】**：
   ...
   - 若偏好“放手一搏”或“高风险”：**核心理念是“要么不中，要么中大的”。不需要刻意去寻找爆冷，可以通过将多场（如3-5场）有把握的比赛串联起来（长串，如4串1、5串1，放弃容错），或者精选2-3场后直接使用【倍投】，以追求极致的奖金回报率。切忌搞复杂比分，保持玩法纯粹。但倍投必须严格遵守 ${budgetAmount}元的预算上限（例如 10 元预算对应 1注 串关时最大只能打 5 倍，绝对不能因为是放手一搏就超预算）。**
...
格式必须严格为：
{
  "howToBuy": "（直接复制发给老板的打票指令。**必须极度精简，只需要具体的打票组合内容，绝对不要任何寒暄、原因分析或多余的文字**。必须写明投了几倍、多少钱，且符合上述预算和乘法计算规则。例如：老板，打个4串1：025让负，026让胜，027让胜，028让负，打5倍（10元）。）",
  "whenToBuy": "（建议购买时间，例如：今晚19:00前，必须在第一场开打前去买）",
  "whyToBuy": "（用大白话解释理由，必须明确写出预计最高能中奖多少钱（并展示赔率相乘乘本金的精确计算过程），以及如果使用了容错，请解释为什么这样容错。绝对不允许凭空捏造和幻觉虚高金额。例如：预计最高中奖约136.9元。计算过程：捷克让负1.93 × 瑞士让胜2.33 × 加拿大让胜1.76 × 墨西哥让负1.73 × 2元 × 5倍 = 136.9元。为了追求高赔，特意选择4场串联且放弃容错以符合放手一搏偏好...）",
  "matches": [
    {
      ...
      try {
        ...
        // 合并预测时的实时赔率信息，方便后期算奖默认带入
        if (parsed.matches && Array.isArray(parsed.matches)) {
          parsed.matches = parsed.matches.map((pm: any) => {
            const original = input.matches.find(m => m.matchNumStr === pm.matchNumStr || (m.homeTeam === pm.homeTeam && m.awayTeam === pm.awayTeam));
            if (original) {
              return {
                ...pm,
                had: original.had,
