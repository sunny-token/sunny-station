...
const fetchYearData = async (
  ...
  if (tasksToFetch.length > 0) {
    const fetchTasks = tasksToFetch.map((item) => {
      return async () => {
        const prizeData = await fetchPrizeDetails("dlt", item.issueNumber);
...
export const dltRouter = router({
  ...
  refreshAll: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      console.log(
        `[DLT] [REFRESH_ALL] ========== 开始一键刷新所有数据 (并发提速版) ==========`,
      );
      const totalStartTime = Date.now();
      const prisma = prismaService.getPrismaClient();
      const startYear = 2007; // 大乐透的最早发行年份
      ...
      for (let currentYear = thisYear; currentYear >= startYear; currentYear--) {
        ...
        yearTasks.push(async () => {
          console.log(`[DLT] [REFRESH_ALL] 开始处理 ${year} 年数据...`);
          ...
          try {
            ...
            if (!isCurrentYear && existingRecords.length >= 140 && needPrizeDetailsSet.size === 0) {
              console.log(`[DLT] [REFRESH_ALL] ${year} 年数据已完备，秒级跳过处理`);
              return;
            }
            ...
            if (existingRecords.length === 0) {
              if (fetchDetails) {
                console.log(
                  `[DLT] [REFRESH_ALL] 冷启动检测到新年份 ${year}，同时开启详情拉取限制 (最多前 ${limit} 期)...`
                );
                ...
                console.log(
                  `[DLT] [REFRESH_ALL] 冷启动检测到新年份 ${year}，仅拉取基础列表以防封禁 IP...`
                );
                yearData = await fetchYearData(
                  year,
              ...
              if (needPrizeDetailsSet.size > limit) {
                ...
                console.log(
                  `[DLT] [REFRESH_ALL] ${year} 年缺失详情较多 (${needPrizeDetailsSet.size} 期)，本轮只补齐前 ${limit} 期，其余将平滑渐进补齐`,
                );
              }
            ...
            if (yearData.length === 0) {
              console.log(`[DLT] [REFRESH_ALL] ${year} 年获取到 0 条数据，跳过`);
              return;
            }
            ...
            console.log(
              `[DLT] [REFRESH_ALL] ${year} 年获取 ${yearData.length} 条数据，其中 ${recordsToProcess.length} 条需要更新 prizeDetailHtml`,
            );
            ...
            if (recordsToProcess.length > 0) {
              ...
              console.log(
                `[DLT] [REFRESH_ALL] ${year} 年处理完成，更新 ${yearCount} 条数据，耗时 ${yearDuration}ms`,
              );
            } else {
              const yearDuration = Date.now() - yearStartTime;
              console.log(
                `[DLT] [REFRESH_ALL] ${year} 年无需更新，耗时 ${yearDuration}ms`,
              );
            }
          } catch (yearError) {
            console.error(`[DLT] [REFRESH_ALL] 处理 ${year} 年数据时发生严重错误:`, yearError);
          }
        });
      }
